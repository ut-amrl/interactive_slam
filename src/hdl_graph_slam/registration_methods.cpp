...
void RegistrationMethods::draw_ui() {
  ...
  ImGui::Combo("Method",
               &registration_method,
               registration_methods.data(),
               registration_methods.size());
  if (std::string(registration_methods[registration_method]).find("NDT") !=
      std::string::npos) {
    ImGui::DragFloat("Resolution", &registration_resolution, 0.1f, 0.1f, 20.0f);
  }
  ImGui::DragFloat(
      "Transformation epsilon", &transformation_epsilon, 1e-5f, 1e-5f, 1e-2f, "%.6f");
  ImGui::DragInt("Max iterations", &max_iterations, 1, 1, 256);
}

pcl::Registration<pcl::PointXYZI, pcl::PointXYZI>::Ptr RegistrationMethods::method()
    const {
  ...
  switch (registration_method) {
    ...
    case 1: {
      auto gicp =
          pcl::GeneralizedIterativeClosestPoint<pcl::PointXYZI, pcl::PointXYZI>::Ptr(
              new pcl::GeneralizedIterativeClosestPoint<pcl::PointXYZI,
                                                        pcl::PointXYZI>);
      registration = gicp;
    } break;
    ...
    case 5: {
      fast_gicp::FastVGICP<pcl::PointXYZI, pcl::PointXYZI>::Ptr vgicp(
