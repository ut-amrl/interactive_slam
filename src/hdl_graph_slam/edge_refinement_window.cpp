...
EdgeInfo::EdgeInfo(g2o::EdgeSE3* edge)
    : edge(edge),
      begin(edge->vertices()[0]->id()),
      end(edge->vertices()[1]->id()),
      num_evaluations(0) {}

bool EdgeInfo::operator<(const EdgeInfo& rhs) const { return error() < rhs.error(); }
...
void EdgeRefinementWindow::draw_ui() {
  ...
  ImGui::DragInt(
      "Optimization cycle", &optimization_cycle, 1, 1, 1024, format.str().c_str());
...
void EdgeRefinementWindow::refinement() {
  ...
  auto loc =
      std::upper_bound(accumulated_error.begin(), accumulated_error.end(), roulette);
  ...
  double fitness_score_before = InformationMatrixCalculator::calc_fitness_score(
      v1->second->cloud, v2->second->cloud, edge.edge->measurement(), 2.0);

  pcl::Registration<pcl::PointXYZI, pcl::PointXYZI>::Ptr registration =
      registration_method.method();

  Eigen::Isometry3d relative =
      v1->second->estimate().inverse() * v2->second->estimate();
  ...
  double fitness_score_after = InformationMatrixCalculator::calc_fitness_score(
      v1->second->cloud, v2->second->cloud, relative, 2.0);
...
void EdgeRefinementWindow::draw_gl(glk::GLSLShader& shader) {
  ...
  auto trans = Eigen::Scaling<float>(1.0f, 1.0f, 1.5f) *
               Eigen::Translation3f(Eigen::Vector3f::UnitZ()) *
               Eigen::AngleAxisf(M_PI, Eigen::Vector3f::UnitX());
  shader.set_uniform(
      "model_matrix",
      (Eigen::Translation3f(pt + Eigen::Vector3f::UnitZ() * 0.1f) * trans).matrix());
  cone.draw(shader);
}
